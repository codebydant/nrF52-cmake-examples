...
#define DEVICE_NAME                                                                                \
  "BLE_Lightbulb" /**< Name of device. Will be included in the advertising                         \
                     data. */
...
#define MIN_CONN_INTERVAL                                                                          \
  MSEC_TO_UNITS(100, UNIT_1_25_MS) /**< Minimum acceptable connection interval (0.1                \
                                      seconds). */
#define MAX_CONN_INTERVAL                                                                          \
  MSEC_TO_UNITS(200, UNIT_1_25_MS) /**< Maximum acceptable connection interval (0.2                \
                                      second). */
...
#define NEXT_CONN_PARAMS_UPDATE_DELAY                                                              \
  APP_TIMER_TICKS(30000) /**< Time between each call to                                            \
                            sd_ble_gap_conn_param_update after the first call                      \
                            (30 seconds). */
...
#define LIGHTBULB_LED                                                                              \
  BSP_BOARD_LED_1 /**< LED to be toggled with the help of the LED Button                           \
                     Service. */
 ...
 * done by simply
 *                setting the disconnect_on_fail config parameter, but instead
 * we use the event
 ...
 */
static void led_write_handler(uint16_t conn_handle, ble_led_service_t *p_led_service,
                              uint8_t led_state) {
  if (led_state) {
...
int main(void) {
  ...
  // log_init();
  timers_init();
  // NRF_LOG_INFO("BLE Lightbulb example started2.");

  // buttons_leds_init(&erase_bonds);
  leds_init();
  // NRF_LOG_INFO("BLE Lightbulb example started3.");
  power_management_init();
  // NRF_LOG_INFO("BLE Lightbulb example started4.");
  ble_stack_init();
  // NRF_LOG_INFO("BLE Lightbulb example started5.");
  gap_params_init();
  // NRF_LOG_INFO("BLE Lightbulb example started6.");
  gatt_init();
  services_init();
  // NRF_LOG_INFO("BLE Lightbulb example started7.");
  advertising_init();
  // NRF_LOG_INFO("BLE Lightbulb example started8.");
  // services_init();
  // NRF_LOG_INFO("BLE Lightbulb example started9.");
  conn_params_init();
  // NRF_LOG_INFO("BLE Lightbulb example started10.");
  // peer_manager_init();

  // Start execution.
  // NRF_LOG_INFO("Template example started.");
  // NRF_LOG_INFO("BLE Lightbulb example started11.");

  application_timers_start();
  // NRF_LOG_INFO("BLE Lightbulb example started12.");
  ...
  // Enter main loop.
  for (;;) {
    // NRF_LOG_INFO("BLE Lightbulb example started - loop.");
    idle_state_handle();
  }
